<template>
  <div>
      ...
      <div class="row">
          ...
          <div class="kyc-info-box">
            <div class="row" v-for="(application, index) in applicationInfo" :key="index">
              ...
              <div class="col-6">
                ...
                <el-popover v-if="application.label === 'Status'" placement="bottom right" width="490"
                            class="status-helper" trigger="hover">
        ...
        <div class="col-xl-4 mb-5">
          <div class="car mt-xl-5 pt-xl-2" style="">
            <regular-table class="kyc-t-card" striped responsive condensed :headings="ApplicationInfoTableHeadings"
                           :value="ApplicationInfoTableValues" :productConfig="true"/>
          </div>
        </div>
        ...
        <div class="row">
            ...
            <div class="kyc__section">
              ...
              <div class="kyc__section__body kyc-info-box--large">
                <div class="row mb-2 pt-3">
                  ...
                  <div class="col-6">
                    <div class="">
                      ...
                      <span v-else-if="getterClientInfo && getterClientInfo.contactInfo"
                            class="color-primary mr-2 word-break">{{ getterClientInfo.contactInfo.email }}</span>
                      <img v-if="!editEmail" :src="editIcon" width="20" @click="editEmail = !editEmail"/>
                      <p-button v-if="editEmail" class="btn btn--view mb-4 mr-2" round @click.stop="goUpdateContact">
                        Update Contact
                      </p-button>
                      <p-button v-if="editEmail" class="btn btn--view mb-4" round @click.stop="cancelEmail">Cancel
                      </p-button>
                    </div>
                  </div>
                </div>
                <div class="row mb-2">
                  ...
                  <div class="col-6">
                    <el-select size="small" :disabled="!editMobile" class="mobile-phone-select mr-3 mb-3"
                               placeholder="Selected A Country Code" v-model="selectedPhoneCode">
                      <el-option v-for="(phoneCode, index) in phoneCodeList" class="select-success"
                                 :value="phoneCode.dial_code" :label="`${phoneCode.name}(${phoneCode.dial_code})`"
                                 :key="index"/>
                    </el-select>
                    ...
                    <p-button v-if="editMobile" class="btn btn--view mb-4 mr-2" round @click.stop="goUpdateContact">
                      Update Contact
                    </p-button>
                    <p-button v-if="editMobile" class="btn btn--view mb-4" round @click.stop="cancelMobile">Cancel
                    </p-button>
                  </div>
                </div>
                <div class="row mb-2"  v-if="hasPermission(permission.KYC_MAIN_PAGE_RESEND_SMS)">
                  <div class="col-6"><p class="kyc-label pt-2">Resend Application SMS</p></div>
                  <div class="col-6">
                    <p-button class="btn btn--view btn--shadow m-0" v-if="getterClientInfo"
                              :disabled="getterClientInfo.isCaptureRequestCompleted" round @click.stop="resendSms">send
                    </p-button>
                  </div>
                </div>
              </div>
          ...
          <div class="col-xl-4 mb-xl-0 mb-5">
            <div class="kyc__section">
              ...
              <div class="">
                <div class="row mb-2"
                     v-if="hasPermission(permission.KYC_MAIN_PAGE_BACK_WORKFLOW)">
                  <div class="col align-center">
                    <p-button round class="btn btn--close btn--shadow"
                              @click.stop.prevent="closeAndReturn">Close and Return</p-button>
                    <p class="mb-0 ml-4">Return to previous menu</p>
                  </div>
                </div>
                <div class="row mb-2"
                     v-if="hasPermission(permission.KYC_MAIN_PAGE_TERMINATE_APP)">
                  <div class="col align-center">
                    <el-popover v-model="showTerminatePopover" placement="left" width="490" class="status-helper"
                                trigger="click">
                      <div class="container-fluid">
                        <div class="row">
                          <div class="col"><h5 class="sub-title text-center mt-4 mb-4">Terminate Application
                            Confirmation</h5></div>
                        </div>
                        <div class="row">
                          <div class="col">
                        ...
                        <div class="row">
                          <div class="col text-center mt-3 mb-4">
                            <p-button round class="btn btn--ac-close mx-2" @click.stop.prevent="closeAC">Close A/C
                            </p-button>
                            <p-button round class="btn btn--ac-cancel mx-2" @click="cancelTerminatePopover">Cancel
                            </p-button>
                          </div>
                        </div>
                      </div>
        ...
        <div class="row">
          <div class="col-xl-4 mb-3">
            <div class="d-flex justify-content-between align-items-center">
              ...
              <p class="status-label" :class="idCheckStatusClass"
                 v-if="getterClientInfo && getterClientInfo.idCheckInfo && getterClientInfo.idCheckInfo.checkStatusName">
                {{ getterClientInfo.idCheckInfo.checkStatusName }}</p>
            </div>
          </div>
        </div>
        ...
        <div class="row mb-3">
          <div class="col-xl-4 mb-xl-0 mb-4">
            <div class="client-info">
              ...
              <el-card class="client-info__card" :body-style="{ padding: '0px' }">
                <el-popover v-if="gettersCheckDocs.ID_FRONT" placement="right" width="550" class="image-zoom"
                            trigger="hover">
                  <img class="w-100"
                       :src="`data:${gettersCheckDocs.ID_FRONT.mimeType};base64, ${gettersCheckDocs.ID_FRONT.content}`">
                  <img class="client-info__card__img" slot="reference" v-if="gettersCheckDocs.ID_FRONT"
                       :src="`data:${gettersCheckDocs.ID_FRONT.mimeType};base64, ${gettersCheckDocs.ID_FRONT.content}`">
                </el-popover>
          ...
          <div class="col-xl-4 mb-xl-0 mb-4">
            <div class="client-info">
              ...
              <el-card class="client-info__card" :body-style="{ padding: '0px' }">
                <el-popover v-if="gettersCheckDocs.ID_BACK" placement="right" width="550" class="image-zoom"
                            trigger="hover">
                  <img class="w-100"
                       :src="`data:${gettersCheckDocs.ID_BACK.mimeType};base64, ${gettersCheckDocs.ID_BACK.content}`">
                  <img class="client-info__card__img" slot="reference" v-if="gettersCheckDocs.ID_BACK"
                       :src="`data:${gettersCheckDocs.ID_BACK.mimeType};base64, ${gettersCheckDocs.ID_BACK.content}`">
                </el-popover>
                <div v-else class="client-info__card__info d-flex justify-content-center align-items-center">
                  <p>No document received</p>
          ...
          <div class="col-xl-4 mb-xl-0 mb-4">
            <div class="client-info">
              ...
              <el-card class="client-info__card" :body-style="{ padding: '0px' }">
                <el-popover v-if="gettersCheckDocs.SELFIE" placement="right" width="550" class="image-zoom"
                            trigger="hover">
                  <img class="w-100"
                       :src="`data:${gettersCheckDocs.SELFIE.mimeType};base64, ${gettersCheckDocs.SELFIE.content}`">
                  <img class="client-info__card__img" slot="reference" v-if="gettersCheckDocs.SELFIE"
                       :src="`data:${gettersCheckDocs.SELFIE.mimeType};base64, ${gettersCheckDocs.SELFIE.content}`">
                </el-popover>
                <div v-else class="client-info__card__info d-flex justify-content-center align-items-center">
                  <p>No document received</p>
        ...
        <div class="row mb-5">
          <div class="col-md-8 kyc-action">
            ...
            <div class="kyc_url_action">
              <p-button round type="primary" class="mr-2 btn btn--view"
                        v-if="hasPermission(permission.KYC_MAIN_PAGE_ID_VIEW)"
                        @click.stop.prevent="goViewId">View</p-button>
              <p-button round class="btn btn--view"
                        v-if="hasPermission(permission.KYC_MAIN_PAGE_ID_ACTION)"
                        @click.stop.prevent="goActionId">Action</p-button>
            </div>
          </div>
          <div class="col-md-4">
            <p class="mb-1 mt-2">
        ...
        <div class="row mb-3 pt-3">
              ...
              <div class="col-xl-6 mb-xl-0 mb-5">
                <div class="client-info">
                  <div class="d-flex justify-content-between align-items-center mb-3">
                    ...
                    <p class="status-label" :class="cardPoaCheckStatus"
                       v-if="getterClientInfo && getterClientInfo.poaCheckInfo && getterClientInfo.poaCheckInfo.checkStatusName">
                      {{ getterClientInfo.poaCheckInfo.checkStatusName }}</p>
                  </div>
                  ...
                  <el-card class="client-info__card" :body-style="{ padding: '0px' }">
                    <el-popover v-if="getterPoaImg" placement="right" width="550" class="image-zoom" trigger="hover">
                      ...
                      <img class="client-info__card__img" slot="reference" v-if="getterPoaImg"
          ...
          <div class="col-xl-4 mb-5">
            ...
            <div class="w-100 d-flex justify-content-between align-items-center">
              ...
              <p class="status-label" :class="cardBtnSancStatus"
                 v-if="getterClientInfo && getterClientInfo.sanctionCheckInfo && getterClientInfo.sanctionCheckInfo.checkStatusName">
                {{ getterClientInfo.sanctionCheckInfo.checkStatusName }}</p>
            </div>
            <el-row>
              <el-col :sm="24" class="kyc-action">
                <p class="mb-3 mt-2 kyc-label kyc-label--no-w">Rescreen date &nbsp;&nbsp;&nbsp;
                  <span v-if="getterClientInfo && getterClientInfo.sanctionCheckInfo">{{ getterClientInfo.sanctionCheckInfo.latestResubmittedDate | dateFormat }}</span>
                </p>
                <div class="kyc_url_action">
                  <p-button round type="primary" class="btn btn--view mr-2"
                            v-if="hasPermission(permission.KYC_MAIN_PAGE_SANCTION_VIEW)"
                            @click.stop.prevent="goSanctionView">View
                  </p-button>
                  <p-button round class="btn btn--view"
                            v-if="hasPermission(permission.KYC_MAIN_PAGE_SANCTION_ACTION)"
                            @click.stop.prevent="goSanctionAction">Action</p-button>
                </div>
              </el-col>
            </el-row>
          </div>
          ...
          <div class="col-xl-4 mb-5">
            <div class="kyc-info-box particulars">
              ...
              <div class="row">
                ...
                <div class="col-6"><p class="kyc-value"><span
                  class="color-primary word-break">{{ particulars.email }}</span></p></div>
              </div>
              <div class="row">
    ...
    <div class="container-fluid">
      ...
      <div class="mt-5">
        <h5 class="sub-title pointer d-inline pl-0 mb-0" @click.stop="toggleCollapsible">Account Log <span
          class="lbl-toggle" ref="collapsibleIcon"></span></h5>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      goActionId() {
        if (this.appReferenceId) this.$router.push({
          name: 'KYC Id Action',
          query: {appRef: this.appReferenceId, action: true}
        })
      },
      goSanctionAction() {
        ...
        if (this.appReferenceId) this.$router.push({
          ...
          query: {appRef: this.appReferenceId, id: id}
        });
      },
      goSanctionView() {
